...
**Настоятельно рекомендуется** перед выполнением задания посмотреть прошедшие лекции ШРИ по интернационализации:

1. [Интернационализация от i до n — ШРИ 2025]

2. [Интернационализация от i до n — ШРИ 2024]
...
3. В URL в виде локали должен учитываться язык и регион пользователя:
    ...
    - в случае если регион пользователя не совпадает с тем, что находится в `URL`, то должен произойти редирект на регион, определенный геобазой (пользователь пришел по ссылке `ru-KZ` из региона `RU`, соответственно должен быть редирект на `ru-RU`)
...
4. В собранном для продакшена бандле приложения не должно быть исходных текстов на русском языке (то есть, при открытии страницы приложения на английском языке в загружаемых данных должны отсутствовать исходные тексты на русском)
...
7. Нужно доработать компонент выбора языка [`LangSelect`](./src/components/lang-select/LangSelect.tsx), чтобы при выборе языка открывалась страница на этом языке
...
- список ключей переводов, используемых на каждой странице, находится в файле [`page-translation-keys.ts`](./src/page-translation-keys.ts)
...
- не изменяйте скрипты `build` и `preview` в `package.json`
...
[Интернационализация от i до n — ШРИ 2025]: https://youtu.be/bcliiAqPuA4
[Интернационализация от i до n — ШРИ 2024]: https://youtu.be/clrC-ltqXk0
